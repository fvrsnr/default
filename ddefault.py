...
class MemoryZipImporter(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    def _load_zip_into_memory(self):
        print(f"[+] Pulling zip into memory: {self.zip_url}")

        with urllib.request.urlopen(self.zip_url, timeout=30) as r:
            zip_bytes = r.read()

        print(f"[+] Zip loaded into memory: {len(zip_bytes)} bytes")
        ...
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
            for name in z.namelist():
                ...
                self.sources[module_name] = {
                    "source": source,
                    "is_package": is_package,
                    "origin": f"memzip://{name}",
                }
        ...
        print("[+] Modules found:")
        for mod in sorted(self.sources):
            print(f"    {mod}")

    def find_spec(self, fullname, path=None, target=None):
        if fullname in self.sources:
            entry = self.sources[fullname]
        ...
        if fullname in self.packages:
            ...
            spec.origin = f"memzip://namespace/{fullname}"
            spec.submodule_search_locations = []
            return spec
    ...
    def exec_module(self, module):
        ...
        if fullname in self.packages and fullname not in self.sources:
            module.__path__ = []
            return

        entry = self.sources[fullname]
        code = compile(entry["source"], entry["origin"], "exec")
...
# ---------------------------------------------------------
# Install memory dependencies BEFORE importing your modules
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Now import your real program dependencies
# ---------------------------------------------------------
